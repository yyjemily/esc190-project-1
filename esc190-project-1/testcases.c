...
void read_in_terms(term **terms, int *pnterms, char *filename) {
    ...
    if (fgets(cur_line, sizeof(cur_line), fp) == NULL) { // handle empty file
        *pnterms = 0;
        *terms = NULL;
        fclose(fp);
        return;
    ...
    *pnterms = atoi(cur_line); // converts string to int
    ...
    if (*pnterms < 0) { // make sure theres a positive number of terms or at least it's 0 (aka nonnegative)
        *terms = NULL;
        fclose(fp);
        return;
    }
...
int lowest_match(term *terms, int nterms, char *substr) {
    // binary search
    if (nterms == 0) {
        return -1; // for arrays with no terms in them
    } 
...
int compweights(const void *a, const void *b) {
    ...
    if (term_a->weight > term_b->weight) {
        ...
        return strcmp(term_a->term, term_b->term); // tie break using lexicogrpahic (wtf is spelling) order
    }
}


void autocomplete(term **answer, int *n_answer, term *terms, int nterms, char *substr) {
    if (nterms == 0 || substr == NULL || strlen(substr) == 0) { // check for all edge cases here
        return;
    }
